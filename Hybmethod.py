...
# get results from SS solution
infile = open('OLG'+Method+'_J'+str(J)+'.pkl', 'rb')
if Method == 'SRA':
...
# unpack paths, note Kpath and Lpath will be used as initial guesses
[Bigc, Bigl, Bigk, Kpath, Lpath, Ypath, rpath, wpath, Fpath, Ipath] = Paths
...
# save AK information
output = open('OLGHyb'+Method+'_J'+str(J)+'.pkl', 'wb')
...
fig.savefig('OLGHyb'+Method+str(J)+'.png', dpi=600)


# plot convergence
fig2 = plt.figure()
plt.plot(Kpathiters[:,0:T].T)
fig2.savefig('OLGHyb'+Method+str(J)+'.png', dpi=600)
