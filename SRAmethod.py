# -*- coding: utf-8 -*-
"""
Created on Mon Nov  7 07:58:15 2022

@author: Kerk
"""

import numpy as np
import pickle as pkl
from RArecalibrate import recalpath
import time
from chainback import allHHplans
import matplotlib.pyplot as plt

##  SETUP
# set number pf ability types
J = 8

# get results from SS solution
infile = open('OLGSS_J'+str(J)+'.pkl', 'rb')
OLGseries, params = pkl.load(infile)
infile.close()

# unpack OLGseries
[cbarmat, ellbarmat, kbarmat, fbarmat, Kbar, Lbar, Cbar, Ybar, \
    Fbar, Ibar, rbar, wbar, fbar] = OLGseries
    
# unpack params
[S, J, beta, gamma, theta, chi, alpha, delta, tau, amat] = params

# set T value (time to SS)
T = 150

# construct productivity matrix over time
amat3 = np.zeros((S,J,T))
for t in range(0,T):
    amat3[:,:,t] = amat[:,0:J]

# set initital state
Bigk0 = .5*kbarmat          ####
K0 = np.sum(Bigk0)
L0 = .75*Lbar               ####

# start timer
tic = time.perf_counter()

##  DO INITIAL CALIBRATION OF RA MODEL
# create targets
CYtar = Cbar/Ybar
Ltar = Lbar/np.sum(amat[:,0:J])
FYtar = Fbar/Ybar
targets = [CYtar, Ltar, FYtar]

##  BEGIN TIME PATH ITERATION
maxit = 100         # maximum number of iterations allows
ccrit = 1.0E-6      # convergence criteria
downscale = .5      # how to change rho if dist rises
upscale = .95       # how to change rho if dist falls
dist = 10000.       # initialize distance measure
olddist = dist
itnum = 0           # initialize iteration number
rho = .2            # initialize damping parameter
Kpath_old = np.ones(T)*Kbar
Lpath_old = np.ones(T)*Lbar
Kpathiters = Kpath_old  # initialize record of Kpath over iterations
Lpathiters = Lpath_old  # initialize record of Lpath over iterations

while(dist>ccrit and itnum<maxit):
    itnum = itnum + 1

    # set guesses for elements of inputs
    guesses = np.array([Kbar/Lbar, beta, chi, tau])

    # relative size of starting value to steady state for K
    relK0OLG = K0/Kbar

    # recalibrate and run new transition path for RA model
    Histlist, RAparams, RAbars = recalpath(targets, guesses, relK0OLG, T, params)

    # unpack histlist
    [KpathRA, LpathRA, zApathRA, YpathRA, wpathRA, rpathRA, FpathRA, CpathRA, \
        IpathRA] = Histlist
        
    # unpack RAbars
    [KbarRA, LbarRA, YbarRA, wbarRA, rbarRA, FbarRA, CbarRA, IbarRA] = RAbars
    
    # rescale paths
    Kpath = KpathRA*(Kbar/KbarRA)
    Lpath = LpathRA*(Lbar/LbarRA)
    rpath = rpathRA*(rbar/rbarRA)
    wpath = wpathRA*(wbar/wbarRA)
    Fpath = FpathRA*(Fbar/FbarRA)
    
    # create paths and SSvecs
    paths = [rpath, wpath, Fpath/(S*J)]
    SSvecs = [cbarmat, ellbarmat, kbarmat] 
    
    # Solve for all household plans
    Bigc, Bigl, Bigk, Bigcheck = allHHplans(paths, SSvecs, Bigk0, T, params)
            
    ##  AGGREGATE AND CHECK FOR CONVERGENCE
    Kpath2 = np.sum(Bigk, axis = (0,1))
    Lpath2 = np.sum(Bigl*amat3, axis = (0,1))
    Kdist = np.max(np.abs(Kpath_old-Kpath2))
    Ldist = np.max(np.abs(Lpath_old-Lpath2)) 
    dist = max(Kdist,Ldist)
    print('itnum: ',itnum,'  dist: ', dist,'  rho: ', rho,'  check max:',\
          np.max(Bigcheck))
    
    # adjust dampener
    if dist > olddist:
        rho = downscale*rho
    else:
        rho = 1 - (1-rho)*upscale
    
    # take convex combination
    Kpath = rho*Kpath2 + (1-rho)*Kpath_old
    Lpath = rho*Lpath2 + (1-rho)*Lpath_old
    Kpathiters = np.vstack((Kpathiters, Kpath))
    Lpathiters = np.vstack((Lpathiters, Lpath))
    olddist = dist
    olddist = dist
    
    # calculate I and C paths
    Ipath = np.zeros(T)
    for t in range(0,T-1):
        Ipath[t] = Kpath[t+1] - delta*Kpath[t]
    Ipath[T-1] = Kbar - delta*Kpath[T-1]    
    Cpath = np.sum(Bigc, axis = (0,1))

    # get remainder of paths
    Ypath = Kpath**alpha * Lpath**(1-alpha)
    rpath = alpha*Ypath/Kpath
    wpath = (1-alpha)*Ypath/Lpath
    Fpath = tau*(wpath*Lpath - (rpath-delta)*Kpath)
    
    # create new targets
    CYtar = Cpath[-1].item(0)/Ypath[-1].item(0)
    Ltar = np.sum(Lpath[-1])/(S*J)
    FYtar = Fpath[-1].item(0)/Ypath[-1].item(0)
    targets = [CYtar, Ltar, FYtar]
    
    Kpath_old = Kpath2
    Lpath_old = Lpath2
    
    
# end timer
toc = time.perf_counter()
elapsed = toc - tic
print('time elapsed: ', elapsed)
    
##  STORE AND ANALYZE RESULTS

# calculate I and C paths
Ipath = np.zeros(T)
for t in range(0,T-1):
    Ipath[t] = Kpath[t+1] - delta*Kpath[t]
Ipath[T-1] = Kbar - delta*Kpath[T-1]    
Cpath = np.sum(Bigc, axis = (0,1))

# get remainder of paths
Ypath = Kpath**alpha * Lpath**(1-alpha)
rpath = alpha*Ypath/Kpath
wpath = (1-alpha)*Ypath/Lpath
Fpath = tau*(wpath*Lpath - (rpath-delta)*Kpath)
fpath = Fpath/(S*J)

# create lists to save
Paths = [Bigc, Bigl, Bigk, Kpath, Lpath, Ypath, rpath, wpath, Fpath, Ipath]
TPIdata = [itnum, dist, elapsed, Kpathiters, Lpathiters, np.max(Bigcheck), \
    ccrit, maxit, downscale, upscale]

# save AK information
output = open('OLGSRA_J'+str(J)+'.pkl', 'wb')
pkl.dump((Paths, TPIdata), output)
output.close()

# plot transition paths
fig = plt.figure()

plt.subplot(3, 2, 1)
plt.plot(Cpath)
plt.xticks([])
plt.title('Consumption', y=.97)

plt.subplot(3, 2, 3)
plt.plot(Kpath)
plt.xticks([])
plt.title('Capital', y=.97)

plt.subplot(3, 2, 5)
plt.plot(Lpath)
plt.title('Labor', y=.97)

plt.subplot(3, 2, 2)
plt.plot(Ypath)
plt.xticks([])
plt.title('GDP', y=.97)

plt.subplot(3, 2, 4)
plt.plot(rpath)
plt.xticks([])
plt.title('Rental Rate', y=.97)

plt.subplot(3, 2, 6)
plt.plot(wpath)
plt.title('Wage', y=.97)
    
fig.savefig('OLGSRAfig_J'+str(J)+'.png', dpi=600)


# plot convergence
fig2 = plt.figure()
plt.plot(Kpathiters[1:-1,0:T].T)
fig2.savefig('OLGSRAfig2_J'+str(J)+'.png', dpi=600)

